...
class KerucutForm extends StatefulWidget {
  const KerucutForm({super.key});

  @override
  State<KerucutForm> createState() => _KerucutFormState();
}
...
class _KerucutFormState extends State<KerucutForm> {
  ...
  double? _hasil;
  String? _caraHasil; // penjelasan perhitungan
  ...
  void _hitung() {
    ...
    if (r > 0 && t > 0) {
      setState(() {
        ...
        _hasil = (1 / 3) * pi * pow(r, 2) * t;

        // buat penjelasan langkah perhitungan
        _caraHasil =
            "V = 1/3 × π × r² × t\n"
            "   = 1/3 × 3.14 × ${r.toStringAsFixed(2)}² × ${t.toStringAsFixed(2)}\n"
            "   = ${_hasil!.toStringAsFixed(2)} cm³";

        // Menambahkan data ke daftar riwayat di urutan paling atas
        _riwayat.insert(0, {
          'jari': r.toString(),
          'tinggi': t.toString(),
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          // Header Card Kerucut dengan Detail Deskripsi
          _buildHeaderCard(
            assetPath: 'assets/kerucut.png',
            title: 'Volume Kerucut',
            formula: 'V = 1/3 × π × r² × t',
            keterangan: 'r = jari-jari alas, t = tinggi kerucut, π = 3.14',
            caraKerja: 'Kerucut memiliki alas lingkaran dan titik puncak. Volume dihitung dengan mengambil sepertiga dari volume tabung (luas alas × tinggi).',
            circleColor: Colors.redAccent,
          ),
          const SizedBox(height: 25),

          const Text('Jari-jari Alas (r)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500)),
          _buildTextField(_rController, 'Masukkan jari-jari'),

          const Text('Tinggi Kerucut (t)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500)),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                const Row(
                  children: [
                    Icon(Icons.straighten, color: Colors.redAccent, size: 18),
                    SizedBox(width: 8),
                    Text(
                      'Rumus:', 
                ...
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Icon(Icons.lightbulb, color: Colors.redAccent, size: 18),
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.history, color: Colors.redAccent, size: 18),
                  ),
                  const SizedBox(width: 15),
                  Expanded(
                    child: Column(
                  ...
                  Text(item['waktu']!, style: const TextStyle(color: Colors.white38, fontSize: 11)),
                ],
              ),
            );
          },
  ...
  Widget _buildResultDisplay() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Hasil Perhitungan:', style: TextStyle(color: Colors.redAccent, fontSize: 14)),
